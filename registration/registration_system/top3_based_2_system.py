...
class Top3Based2System(Top3PrioritySystem):
    def __init__(self, courses: Dict[str, Course]) -> None:
        super().__init__(courses)

    def register_students(self, students: List[Student], p: Tuple, major_guarantee: float, grade_guarantee: float,
                          graduate_standard: int) -> List[Student]:
        ...
        for code, register_info in self.registration_dict.items():
            ...
            assert course.get_num_applicants() == len(
                register_list), "Number of applicants should be equal to number of students"
            ...
            if not course.is_lottery:
                ...
                top3_based_2 = {"first": [], "second": [], "third": [], "guarantee": [], "etc": []}

                for student in register_list:
                    for priority, student_course in enumerate(student.timetable):
                        if student_course.code == code:
                            if priority == 0:
                                top3_based_2["first"].append(student)
                            elif priority == 1:
                                top3_based_2["second"].append(student)
                            elif priority == 2:
                                top3_based_2["third"].append(student)
                            else:
                                top3_based_2["etc"].append(student)

                            continue

                first_capacity = min(int(course.capacity * first_probability), len(top3_based_2["first"]))
                second_capacity = min(int(course.capacity * second_probability), len(top3_based_2["second"]))
                third_capacity = min(int(course.capacity * third_probability), len(top3_based_2["third"]))
                ...
                for i, student in enumerate(top3_based_2["first"]):
                    ...
                    if guarantee_benefitted < int(first_capacity * 0.5):
                        ...
                        remained_students.extend(top3_based_2["first"][i:])
                        break

                random.shuffle(remained_students)
                for (i, student) in enumerate(remained_students):
                    if i < (first_capacity - guarantee_benefitted):
                        student.add_to_final_timetable(course)
                    else:
                        del remained_students[:i]
                        break

                top3_based_2["second"].extend(remained_students)
                remained_students = []
                guarantee_benefitted = 0
                ...
                for i, student in enumerate(top3_based_2["second"]):
                    ...
                    if guarantee_benefitted < int(second_capacity * 0.5):
                        if is_major and is_grade and (more_major or more_grade):
                            student.add_to_final_timetable(course)
                            major_headcount += 1
                            grade_headcount += 1
                        ...
                        remained_students.extend(top3_based_2["second"][i:])
                        break

                random.shuffle(remained_students)
                for (i, student) in enumerate(remained_students):
                    if i < (second_capacity - guarantee_benefitted):
                        student.add_to_final_timetable(course)
                    else:
                        del remained_students[:i]
                        break

                top3_based_2["third"].extend(remained_students)
                remained_students = []
                guarantee_benefitted = 0
                ...
                for i, student in enumerate(top3_based_2["third"]):
                    ...
                    if guarantee_benefitted < int(third_capacity * 0.5):
                        if is_major and is_grade and (more_major or more_grade):
                            student.add_to_final_timetable(course)
                            major_headcount += 1
                            grade_headcount += 1
                        ...
                        remained_students.extend(top3_based_2["third"][i:])
                        break

                random.shuffle(remained_students)
                for (i, student) in enumerate(remained_students):
                    if i < (third_capacity - guarantee_benefitted):
                        student.add_to_final_timetable(course)
                    else:
                        del remained_students[:i]
                        break
                ...
                top3_based_2["guarantee"].extend(remained_students)
                random.shuffle(top3_based_2["guarantee"])

                guarantee_benefitted = 0
                for student in top3_based_2["guarantee"]:
                ...
                random.shuffle(top3_based_2["etc"])
                for i, student in enumerate(top3_based_2["etc"]):
